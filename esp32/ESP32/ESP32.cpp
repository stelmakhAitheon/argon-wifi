...
#define ESP32_DEFAULT_BAUD_RATE   921600
...
void ESP32::_startup_common()
{
    ...
    _serial.set_flow_control(SerialBase::RTSCTS, P1_7, P1_6);
...
bool ESP32::open(const char *type, int id, const char* addr, int port, int opt)
{
    ...
    memset(cmd, 0, 1000);
    if (opt != 0) {
        snprintf(cmd, sizeof(cmd), "AT+CIPSTART=%d,\"%s\",\"%s\",%d,%d", id, type, addr, port, opt);
    } else {
    ...
    ret = true;
    // _init_end_common = false;
    // _startup_wifi();
    // _parser.send("AT");
    // bool res = _parser.recv("OK");
    // sendChannel(uint8_t channel, uint8_t control, bool cmd, const uint8_t* data, size_t len);
    //end my code
...
bool ESP32::send(int id, const void *data, uint32_t amount)
{
    ...
    if(!first)
        return false;
    first = false;
    // uint8_t sendData[amount+2];
    // memcpy(sendData, data, amount);
    // sendData[amount-2] = '\r';
    // sendData[amount-1] = '\n';
    
    // Muxer *muxer = new Muxer(&_serial);
    // uint8_t cmd[] = "AT+GETMAC=0\r\n";
    // sendChannel(1, 239, false, cmd, sizeof(cmd));
    // // Thread *thread = new Thread();
    // // thread.start([&_serial] {
