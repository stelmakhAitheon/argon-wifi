...
inline int Muxer<StreamT, MutexT>::ping() {
    ...
    auto bits = events_.wait_any(EVENT_PING, getControlResponseTimeout(), true);
    if (bits & EVENT_PING) {
        return 0;
    }
    ...
    if ((chan->remoteModemState & (proto::RTR | proto::FC)) == proto::RTR) {
        ...
        while ((portable::getMillis() - t1) < timeout && isRunning() &&
            ...
            if (toWait > 0) {
                ...
                channelEvents_.wait_any(EVENT_STATE_CHANGED << chan->channel, toWait, true);
                if ((chan->remoteModemState & (proto::RTR | proto::FC)) == proto::RTR) {
                    return 0;
                }
            }
...
// #endif // GSM0710_RELAX_WHEN_CHANNEL_IN_FLOW_CONTROL
            ...
            if (channelDataResult == GSM0710_ERROR_FLOW_CONTROL) {
                ...
                // Waiting for a wakeup or up to GSM0710_RELAX_WHEN_CHANNEL_IN_FLOW_CONTROL ms
                // xEventGroupWaitBits(events_, EVENT_WAKEUP, pdFALSE, pdFALSE, GSM0710_RELAX_WHEN_CHANNEL_IN_FLOW_CONTROL / portTICK_RATE_MS);
                events_.wait_any(EVENT_WAKEUP, 1000, false);
            }   
        }
    }
...
inline int Muxer<StreamT, MutexT>::processModemState(const uint8_t* data, size_t length) {
    ...
    if (c && l >= 1) {
        ...
        channelEvents_.set(EVENT_STATE_CHANGED << channel);
    }
...
template<typename StreamT, typename MutexT>
bool Muxer<StreamT, MutexT>::waitStream(uint64_t millis) {
    volatile bool wait = true;
    Timeout out;
    out.attach_us(callback((void (*)(volatile bool *))[](volatile bool *wait) {
        *wait = false; 
    }, &wait), millis * 1000);
    while(wait && !stream_->writable());
    return wait;
}
...
inline int Muxer<StreamT, MutexT>::sendChannel(uint8_t channel, uint8_t control, bool cmd, const uint8_t* data, size_t len) {
    ...
    while (sent < hlen) {
        ...
        CHECK_TRUE(waitStream(getControlResponseTimeout() * 2), -1);
    }
...
inline int Muxer<StreamT, MutexT>::channelTransition(Channel* channel, ChannelState state) {
    ...
    channelEvents_.set(EVENT_STATE_CHANGED << channel->channel);
