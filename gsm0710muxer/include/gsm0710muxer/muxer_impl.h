...
inline int Muxer<StreamT, MutexT>::start(bool initiator, bool openZeroChannel) {
    ...
    //         }
        ...
        // Thread
        // thread_.start([] {
        //     Logger::getInstance()->addMessage("controlProcess timeout \r\n");
        //     // this->run();    
        // }); //<StreamT, MutexT>
        thread_.start(this, &Muxer::runThread);
...
inline int Muxer<StreamT, MutexT>::stop() {
    ...
    // addMessage("Stopping GSM07.10 muxer", true);
    ...
    // addMessage("GSM07.10 muxer stopped", true);
...
inline int Muxer<StreamT, MutexT>::openChannel(uint8_t channel, ChannelDataHandler handler, void* ctx) {
    ...
    if (c->state == ChannelState::Closed || c->state == ChannelState::Closing || c->state == ChannelState::Error) {
        ...
        LOG(INFO, "Openning mux channel %u", channel);
        {
            ...
            c->retries = 0;
            if (handler != nullptr) {
                // addMessage("openChannel handler", true);
                c->handler = handler;
                c->handlerCtx = ctx;
            }
        }
        // addMessage("openChannel waitChannelState", true);
...
template<typename StreamT, typename MutexT>
void Muxer<StreamT, MutexT>::runThread() {
    Logger::getInstance()->addMessage("runThread \r\n");
    run();
}

template<typename StreamT, typename MutexT>
inline void Muxer<StreamT, MutexT>::run() {
...
inline int Muxer<StreamT, MutexT>::processTimeouts() {
    ...
    events_.clear(EVENT_WAKEUP);

    if (!initiator_ && getChannel(0)->state != ChannelState::Opened && !isStopping()) {

        // FIXME: for now returning T1 / 2
        // addMessage("processTimeouts FAIL", true);
        // if (toParse() > 0) { //need solve
        //     return 1;
        // }
        // return getAckTimeout() / 2; //need solve
    }

    // std::lock_guard<MutexT> lk(mutex_); //need solve


    int timeout = CHECK(controlProcess());//need solve
    
    // addMessage("processTimeouts c = " + std::to_string(sizeof(channels_)) + " 0 = " + std::to_string(sizeof(channels_[0])), true);
    
    int len = sizeof(channels_) / sizeof(channels_[0]);
    // addMessage("processTimeouts len = " + std::to_string(len), true);
    Logger::getInstance()->addMessage("processTimeouts len = %d\r\n", len);
    int n = 0;
    for (int c = 0; c < 5; c++) {
        n++;
        // auto chan = &channels_[c];
        // int ch = c;
        // addMessage("processTimeouts c = " + std::to_string(c), true);
        // Logger::getInstance()->addMessage("processTimeouts c = %d\r\n", c);
    
        // Logger::getInstance()->addMessage("processTimeouts c = %d", c);
        // ThisThread::sleep_for(1000);
        
        // if (chan->state == ChannelState::Closing || chan->state == ChannelState::Opening) {
    //         // addMessage("processTimeouts ChannelState close or openning", true);
    //         // Transitive state need solve
    ...
    //         // addMessage("processTimeouts ChannelState Opened", true);
    //         // if (ctrl_.state != ControlCommand::State::Pending) { // need solve
    //         //     chan->update = false;
...
inline int Muxer<StreamT, MutexT>::controlProcess() {
    ...
    // addMessage("controlProcess timeout = " + std::to_string(timeout), true);
    // Logger::getInstance()->addMessage("controlProcess timeout \r\n");
    // Logger::getInstance()->addMessage("controlProcess timeout = %d\r\n", timeout);
    if (ctrl_.state == ControlCommand::State::Pending) {
        // addMessage("controlProcess pending", true);
        // Logger::getInstance()->addMessage("controlProcess pending\r\n");
        // if ((portable::getMillis() - ctrl_.timestamp) >= getControlResponseTimeout()) { //need solve
        //     // Time to retry
